        ...
        <div class="mws-form-row"> 
         ...
         <div class="mws-form-item clearfix"> 
          ...
          <ul class="mws-form-list inline">
              @foreach($role as $row) 
              <li><input type="checkbox" name="rids[]" value="{{$row->id}}" @if($row->status==0) disabled @endif  @if(in_array($row->id,$rids)) checked @endif> <label>{{$row->name}}</label></li> 
              @endforeach
          </ul> 
         </div> 
        </div> 
